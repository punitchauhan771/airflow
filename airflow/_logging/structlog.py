...
from typing import Any, BinaryIO, Callable, Generic, Protocol, TextIO, TypeVar, Union

import pygtrie
import structlog
from structlog.processors import NAME_TO_LEVEL
from structlog.typing import EventDict, ExcInfo, FilteringBoundLogger, Processor, WrappedLogger
...
def make_filtering_logger(default_min_level: int | str) -> Callable[..., AirflowFilteringBoundLogger]:
    ...
    def maker(logger: WrappedLogger, *args, **kwargs):
        ...
        if isinstance(logger, (NamedWriteLogger, NamedBytesLogger)):
            ...
            level = PER_LOGGER_LEVELS.longest_prefix(logger_name).get(default_min_level)
        else:
...
class LoggerFactory(Generic[LogOutputType]):
    def __init__(self, cls: Callable[[str, LogOutputType | None]], io: LogOutputType | None = None):
        self.cls = cls
        self.io = io

    def __call__(self, logger_name: str | None = None, *args: Any) -> LogOutputType:
...
# `eyJ` is `{"}` in base64 encoding -- i.e. and any value that starts like that is in high likely hood a JWT
